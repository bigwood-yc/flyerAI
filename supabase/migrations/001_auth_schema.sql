...
-- Users can update only their own row; WITH CHECK prevents changing is_whitelisted
CREATE POLICY "users_update_own_profile" ON public.user_profiles
  FOR UPDATE
  USING (auth.uid() = id)
  WITH CHECK (
    auth.uid() = id
    AND is_whitelisted = (SELECT is_whitelisted FROM public.user_profiles WHERE id = auth.uid())
  );
